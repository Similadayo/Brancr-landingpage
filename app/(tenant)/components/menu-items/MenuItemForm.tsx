...
export default function MenuItemForm({ item }: MenuItemFormProps) {
  ...
  const handleParse = async () => {
    ...
    if (items && items.length > 0) {
      ...
      toast.error("Could not extract menu item details.");
    }
  };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl dark:bg-gray-800">
            ...
            <p className="mb-4 text-sm text-gray-600 dark:text-gray-300">
              Paste your menu item details (e.g. from WhatsApp or a document) and AI will extract the structured data for you.
            </p>
            <textarea
              value={parseInput}
              onChange={(e) => setParseInput(e.target.value)}
              placeholder="e.g. Jollof Rice with Chicken - ₦3,500. Spicy and delicious. Ready in 20 mins."
