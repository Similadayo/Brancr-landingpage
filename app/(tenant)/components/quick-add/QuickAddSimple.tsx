...
export default function QuickAddSimple() {
  ...
  const handleParse = async () => {
    ...
    try {
      setParsing(true);
      const res = await fetch('/api/tenant/products/parse', {
        method: 'POST',
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <input type="file" accept=".csv,.txt,.docx" onChange={async (e) => {
          ...
          try {
            setParsing(true);
            const res = await fetch('/api/tenant/products/parse/file', { method: 'POST', body: form });
            if (res.status === 202) {
      ...
      <div>
        ...
        {parsed ? (
          <ParsedItemsReview items={parsed} onSaved={() => setParsed(null)} />
        ) : (
