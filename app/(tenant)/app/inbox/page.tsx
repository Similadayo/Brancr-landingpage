...
const SAMPLE_CONVERSATIONS: Conversation[] = [
  ...
  {
    id: "3",
    contactName: "Sparkle Lounge",
    channel: "facebook",
    preview: "Appreciate the shoutout! Let’s talk collaborations.",
    timestamp: "Yesterday",
    unread: false,
    tags: ["Campaign"],
    status: "closed",
  ...
  "3": [
    {
      ...
      author: "tenant",
      name: "Tunde (You)",
      body: "We’d love to partner! Could you share audience stats from your last campaign?",
      time: "Yesterday",
    },
    {
      id: "m6",
      author: "contact",
      name: "Sparkle Lounge",
      body: "Attached the deck. Let me know if you need extras.",
      time: "Yesterday",
    },
  ],
};
...
const CHANNEL_COLORS: Record<Conversation["channel"], string> = {
  whatsapp: "bg-emerald-100 text-emerald-700",
  instagram: "bg-fuchsia-100 text-fuchsia-700",
  facebook: "bg-blue-100 text-blue-700",
  tiktok: "bg-neutral-900 text-white",
...
export default function InboxPage() {
  const { tenant } = useTenant();
  const [activeFilter, setActiveFilter] = useState<string>("All");
  const [selectedConversation, setSelectedConversation] = useState<string>(SAMPLE_CONVERSATIONS[0]?.id ?? "");

  const filteredConversations = useMemo(() => {
    if (activeFilter === "All") return SAMPLE_CONVERSATIONS;
    return SAMPLE_CONVERSATIONS.filter((conversation) => conversation.status.toLowerCase() === activeFilter.toLowerCase());
  }, [activeFilter]);

  const messages = selectedConversation ? SAMPLE_MESSAGES[selectedConversation] ?? [] : [];

  const activeConversation = SAMPLE_CONVERSATIONS.find((conv) => conv.id === selectedConversation);
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="text-right">
                    <p className="text-[10px] uppercase tracking-widest text-gray-400">{conversation.timestamp}</p>
                    {conversation.unread ? (
                      <span className="mt-2 inline-flex h-6 w-6 items-center justify-center rounded-full bg-primary text-xs font-semibold text-white">
                        1
                      </span>
        ...
        {activeConversation ? (
          <>
            <header className="flex flex-col gap-3 border-b border-gray-200 pb-4 md:flex-row md:items-center md:justify-between">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <button className="rounded-xl border border-gray-200 px-3 py-2 text-xs font-semibold text-gray-600 transition hover:border-primary hover:text-primary">
                  Add note
                </button>
                <button className="rounded-xl border border-gray-200 px-3 py-2 text-xs font-semibold text-gray-600 transition hover:border-primary hover:text-primary">
                  Assign
                </button>
                <button className="rounded-xl bg-primary px-3 py-2 text-xs font-semibold text-white shadow-md shadow-primary/20 hover:bg-primary/90">
                  Close conversation
                </button>
              </div>
            </header>
              ...
              <div className="flex flex-1 flex-col gap-4">
                <div className="flex-1 space-y-4 overflow-y-auto rounded-xl bg-neutral-bg p-4">
                  {messages.map((message) => (
                    ...
                    >
                      <header className="flex items-center justify-between">
                        <p className="text-xs font-semibold text-gray-500">{message.name}</p>
                        <span className="text-[10px] uppercase tracking-[0.3em] text-gray-400">{message.time}</span>
                      </header>
                ...
                <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
                  ...
                  <div className="mt-3 space-y-3">
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      ...
                      <button className="inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-2 text-sm font-semibold text-white shadow-md shadow-primary/20 hover:bg-primary/90">
                        Send reply
                      </button>
                    </div>
                  </div>
              ...
              <aside className="w-full rounded-2xl border border-gray-200 bg-white p-4 shadow-sm lg:h-full lg:w-80">
                ...
                <div className="mt-4 space-y-4 text-xs text-gray-600">
                  ...
                  <div>
                    ...
                    <div className="mt-2 rounded-xl border border-gray-200 bg-white p-3 text-sm text-gray-600">
                      <p>Coordinate delivery with logistics team. Customer prefers morning drops.</p>
                      <button className="mt-2 inline-flex items-center gap-1 text-xs font-semibold text-primary hover:text-primary/80">
                        Add note <span aria-hidden>+</span>
                      </button>
                    </div>
                  </div>
